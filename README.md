...
- **v0.10.3 播放热连接池**：媒体拥有独立于 API 的**保活连接池**——首播暖起来的上游连接会被后续分片和每次 Range 拖动复用，不再每次重跑 TCP 慢启动（此前"只显示几十 KB 迟迟提不起速"的根因）；控制流仍走独立的 API 池，两池互不污染。若某些前端在复用连接后只回传几十 KB 就停顿，隐形字节续拉会立刻改用一次性干净连接兜底，因此保活既拿到暖窗口的速度、又不会把播放卡死在异常源上。中途断流的续拉预算改为**按进度返还**：只要仍在推进，两小时的影片遇到再多零星卡顿也不会耗尽预算而断流；失活看门狗由 8 秒放宽到 15 秒，避免把边转码边缓冲的慢源误判为断流。主代理与探针共用同一逻辑。
- **v0.10.5 复用连接快速退池（全程同线路，不切换机器/上游）**：① 续拉护栏——推进满 512 KiB 才返还重试预算，另加"每分钟≤8 次、单次播放≤48 次"硬上限，异常源小口喂几字节也无法把续拉循环拖死；达上限即 `destroy` 下游让播放器明确断流重连，绝不假装播完。② `reusedSocket` 精准同线重试——只有确实复用了池中旧 socket 且 `ECONNRESET`/`EPIPE`，或复用连接在短租约内没响应，才在原线路干净重试一次；超时/全新 socket 一律不重复请求，单上游最坏等待从约 90 秒降到约 45 秒。③ 复用连接响应头前 8 秒快速退池；④ 首播前 512 KiB 对复用 socket 用 8 秒短失活检测，超时立即改用干净连接续拉，直击"只出几十 KB 迟迟不提速"——干净连接与慢转码源仍保留 45 秒/15 秒的完整容忍。主代理与探针共用同一逻辑。
- **v0.10.8 探针安装幂等 + 真实健康上报**：在已注册的机器上重跑安装命令不再抛「agent is already enrolled」堆栈——检测到 identity 即跳过注册、只更新代码与容器（一次性令牌已作废也无妨）；身份文件本就在控制面确认注册之后才原子落盘，不存在"文件在、控制面没有"的半注册态。探针上报的 `proxyHealthy` 改为真实值（代理监听器出错/关闭即置假并记日志，不再永远显示健康），优雅退出增加 `closeIdleConnections` + 强制超时，避免长连接把 `docker compose down` 拖到 10 秒强杀。
