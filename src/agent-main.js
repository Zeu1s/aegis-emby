...
import { handleUpgrade, makeProxyHandler, RELAY_SERVER_OPTIONS } from './proxy.js';
...
// The agent runs unprivileged in a container, so it cannot touch host Nginx or certbot. It drops a
// validated request file into the mounted data dir; a narrowly scoped host unit performs the switch.
function requestDomainSwitch(domain,email){
  const desiredDomain=String(domain||'').trim().toLowerCase(),certificateEmail=String(email||'').trim().toLowerCase();
...
function startProxy(config,identity,metricStore,health){const metrics=new Metrics(metricStore),key=storageKey(identity),relay=makeProxyHandler(config.routeSource,key,metrics),server=http.createServer(RELAY_SERVER_OPTIONS,(req,res)=>{res.on('error',()=>{});try{const out=relay(req,res);if(out&&typeof out.catch==='function')out.catch(error=>{console.error('[aegis-agent] relay error',error?.message||error);if(!res.headersSent&&!res.writableEnded){res.writeHead(502,{'cache-control':'no-store'});res.end('bad gateway')}})}catch(error){if(!res.headersSent&&!res.writableEnded){res.writeHead(502,{'cache-control':'no-store'});res.end('bad gateway')}}});server.requestTimeout=0;server.keepAliveTimeout=75_000;server.keepAliveTimeoutBuffer=5_000;server.headersTimeout=80_000;server.on('connection',socket=>{socket.setNoDelay(true);socket.setKeepAlive(true,15_000)});server.on('clientError',(_error,socket)=>{if(!socket.destroyed)socket.destroy()});server.on('upgrade',(req,socket,head)=>{socket.on('error',()=>socket.destroy());try{handleUpgrade(req,socket,head,config.routeSource,key)}catch{socket.destroy()}});return new Promise((resolve,reject)=>{let started=false;server.on('error',error=>{if(!started)return reject(error);health.proxyHealthy=false;console.error(`[aegis-agent] proxy server error: ${error?.message||error}`)});server.on('close',()=>{health.proxyHealthy=false});server.listen(PROXY_PORT,'0.0.0.0',()=>{started=true;resolve({server,metrics})})})}
